...
def simgle_f_cal_mask_time_optimized(input_data, num_samples=2000):
    """
    使用采样法的优化版本计算单个无人机的遮蔽时间（速度更快）

    Args:
        input_data: (f_num, f_vx, f_vy, drop_t, bomb_t) 输入参数
...
def merge_intervals_q4(intervals):
    """
    合并重叠的时间区间（Q4版本）
...
def Q4_cal_mask_time_optimized(input_data, num_samples=2000):
    """
    使用采样法的优化版本计算Q4的总遮蔽时间（速度更快）

    Args:
        input_data: (f1_vx, f1_vy, f2_vx, f2_vy, f3_vx, f3_vy, drop_t1, bomb_t1, drop_t2, bomb_t2, drop_t3, bomb_t3) 输入参数
        num_samples: 采样点数量，默认2000
    ...
    # 计算三个无人机的遮蔽区间
    intervals1 = simgle_f_cal_mask_time_optimized(
        (1, f1_vx, f1_vy, drop_t1, bomb_t1), num_samples
    )
    intervals2 = simgle_f_cal_mask_time_optimized(
        (2, f2_vx, f2_vy, drop_t2, bomb_t2), num_samples
    )
    intervals3 = simgle_f_cal_mask_time_optimized(
        (3, f3_vx, f3_vy, drop_t3, bomb_t3), num_samples
    )
    ...
    # 计算合并后的总时长
    return merge_intervals_q4(all_intervals)
